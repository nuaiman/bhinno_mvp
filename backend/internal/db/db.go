package db

import (
	"backend/internal/config"
	"context"
	"log"
	"time"

	"github.com/jackc/pgx/v5/pgxpool"
)

var Pool *pgxpool.Pool

func Init(cfg *config.Config) *pgxpool.Pool {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	pool, err := pgxpool.New(ctx, cfg.DB_URL)
	if err != nil {
		log.Fatalf("Failed to connect to Postgres: %v", err)
	}

	if err := pool.Ping(ctx); err != nil {
		log.Fatalf("Failed to ping Postgres: %v", err)
	}

	log.Println("Connected to Postgres successfully")
	Pool = pool

	createTables(ctx)

	return Pool
}

func Close() {
	if Pool != nil {
		Pool.Close()
		log.Println("Postgres pool closed")
	}
}

func createTables(ctx context.Context) {
	tables := []string{
		`CREATE TABLE IF NOT EXISTS users (
			id BIGSERIAL PRIMARY KEY,
			verified BOOLEAN DEFAULT FALSE,
			role VARCHAR(25) NOT NULL DEFAULT 'client'
				CHECK (role IN ('client', 'server', 'superadmin')),
			status VARCHAR(25) NOT NULL DEFAULT 'review'
				CHECK (status IN ('active', 'review', 'suspended', 'banned')),
			name VARCHAR(255),
			phone VARCHAR(25) UNIQUE,
			avatar VARCHAR(500),
			bio VARCHAR(255),
			email VARCHAR(255) UNIQUE,
			password TEXT, -- Use TEXT for hashed passwords (e.g., Argon2/Bcrypt)
			reset_token VARCHAR(255),
			reset_token_expiry TIMESTAMPTZ,
			google_id VARCHAR(255) UNIQUE,
			google_id_token TEXT,
			google_access_token TEXT,
			refresh_token TEXT,
			refresh_token_at TIMESTAMPTZ,
			rating_avg NUMERIC(3,2) NOT NULL DEFAULT 0.00,
			rating_count INT NOT NULL DEFAULT 0,
			created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
		);`,

		`CREATE TABLE IF NOT EXISTS categories (
			id BIGSERIAL PRIMARY KEY,
			name VARCHAR(255) NOT NULL,
			description VARCHAR(255) NOT NULL,
			created_at TIMESTAMPTZ DEFAULT NOW()
		);`,

		`CREATE TABLE IF NOT EXISTS sub_categories (
			id BIGSERIAL PRIMARY KEY,
			category_id BIGINT NOT NULL REFERENCES categories(id) ON DELETE CASCADE,
			name VARCHAR(255) NOT NULL,
			description VARCHAR(255) NOT NULL,
			created_at TIMESTAMPTZ DEFAULT NOW()
		);`,

		`CREATE TABLE services (
			id BIGSERIAL PRIMARY KEY,
			active BOOLEAN NOT NULL DEFAULT TRUE,
			user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
			division_id INT NOT NULL,
			district_id INT NOT NULL,
			subdistrict_id INT NOT NULL,
			area VARCHAR(255) NOT NULL,
			category_id BIGINT NOT NULL REFERENCES categories(id) ON DELETE RESTRICT,
			subcategory_id BIGINT NOT NULL REFERENCES sub_categories(id) ON DELETE RESTRICT,
			title VARCHAR(255) NOT NULL,
			short_description VARCHAR(144) NOT NULL,
			description VARCHAR(1000) NOT NULL,
			price VARCHAR(255) NOT NULL,
			features JSONB CHECK (features IS NULL OR (jsonb_typeof(features) = 'object' AND length(features::text) <= 4096)),
			hours VARCHAR(50) CHECK (hours IS NULL OR hours = 'All day' OR hours ~ '^([01]?[0-9]|2[0-3]):[0-5][0-9]-([01]?[0-9]|2[0-3]):[0-5][0-9]$'),
			days TEXT[] NOT NULL CHECK (ARRAY(SELECT unnest(days) EXCEPT SELECT unnest(ARRAY['mon','tue','wed','thu','fri','sat','sun'])) = '{}' AND length(array_to_string(days,',')) <= 50),
			created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
		);`,

		`CREATE TABLE bookings (
			id BIGSERIAL PRIMARY KEY,
			service_id BIGINT NOT NULL REFERENCES services(id) ON DELETE CASCADE,
			user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,       
			provider_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE, 
			hours VARCHAR(50) CHECK (hours IS NULL OR hours = 'All day' OR hours ~ '^([01]?[0-9]|2[0-3]):[0-5][0-9]-([01]?[0-9]|2[0-3]):[0-5][0-9]$'),
			days TEXT[] NOT NULL CHECK (ARRAY(SELECT unnest(days) EXCEPT SELECT unnest(ARRAY['mon','tue','wed','thu','fri','sat','sun'])) = '{}' AND length(array_to_string(days,',')) <= 50),
			status VARCHAR(25) NOT NULL DEFAULT 'pending' 
				CHECK (status IN ('pending', 'confirmed', 'completed', 'cancelled')),
			created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
		);`,

		`CREATE TABLE ratings (
			id BIGSERIAL PRIMARY KEY,
			user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
			provider_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
			service_id BIGINT NOT NULL REFERENCES services(id) ON DELETE CASCADE,
			rating NUMERIC(3,2) NOT NULL CHECK (rating >= 0 AND rating <= 5),
			comment VARCHAR(500),
			created_at TIMESTAMPTZ DEFAULT NOW()
		);`,
	}

	for _, q := range tables {
		if _, err := Pool.Exec(ctx, q); err != nil {
			log.Fatalf("Failed to create table: %v", err)
		}
	}

	log.Println("All tables ensured")

	indexes := []string{
		// users
		`CREATE INDEX IF NOT EXISTS idx_users_phone ON users(phone);`,
		`CREATE INDEX IF NOT EXISTS idx_users_role ON users(role);`,
		`CREATE INDEX IF NOT EXISTS idx_users_refresh_token ON users(refresh_token);`,

		// sub_categories
		`CREATE INDEX IF NOT EXISTS idx_sub_categories_category_id ON sub_categories(category_id);`,

		// services
		`CREATE INDEX IF NOT EXISTS idx_services_user_id ON services(user_id);`,
		`CREATE INDEX IF NOT EXISTS idx_services_active ON services(active);`,
		`CREATE INDEX IF NOT EXISTS idx_services_category_id ON services(category_id);`,
		`CREATE INDEX IF NOT EXISTS idx_services_subcategory_id ON services(subcategory_id);`,
		`CREATE INDEX IF NOT EXISTS idx_services_created_at ON services(created_at);`,
		`CREATE INDEX IF NOT EXISTS idx_services_rating ON services(rating_avg);`,

		// Partial index for active services (most common filter)
		`CREATE INDEX IF NOT EXISTS idx_services_active_true ON services(created_at DESC) WHERE active = TRUE;`,

		// Location-based filtering
		`CREATE INDEX IF NOT EXISTS idx_services_location ON services(division_id, district_id, subdistrict_id);`,

		// Category + active combo
		`CREATE INDEX IF NOT EXISTS idx_services_active_category ON services(category_id) WHERE active = TRUE;`,

		// Composite index for category + location + active (likely query)
		`CREATE INDEX IF NOT EXISTS idx_services_active_category_location 
        ON services(category_id, division_id, district_id, subdistrict_id) 
        WHERE active = TRUE;`,

		// GIN index on features JSONB
		`CREATE INDEX IF NOT EXISTS idx_services_features ON services USING GIN(features);`,

		// Optional: available_days GIN index if filtering by day
		`CREATE INDEX IF NOT EXISTS idx_services_available_days ON services USING GIN(available_days);`,

		// -----------------------
		// service_bookings indexes
		// -----------------------

		// Filter bookings by client
		`CREATE INDEX IF NOT EXISTS idx_bookings_user_id ON service_bookings(user_id);`,

		// Filter bookings by provider
		`CREATE INDEX IF NOT EXISTS idx_bookings_provider_id ON service_bookings(provider_id);`,

		// Filter bookings by service
		`CREATE INDEX IF NOT EXISTS idx_bookings_service_id ON service_bookings(service_id);`,

		// Filter by booking status
		`CREATE INDEX IF NOT EXISTS idx_bookings_status ON service_bookings(status);`,

		// Filter bookings by time range
		`CREATE INDEX IF NOT EXISTS idx_bookings_time ON service_bookings(start_time, end_time);`,
	}

	for _, i := range indexes {
		if _, err := Pool.Exec(ctx, i); err != nil {
			log.Fatalf("Failed to create index: %v", err)
		}
	}

	log.Println("All indexes ensured")
}
